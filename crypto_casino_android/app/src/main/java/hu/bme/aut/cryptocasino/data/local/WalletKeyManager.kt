package hu.bme.aut.cryptocasino.data.local
...
class WalletKeyManager
    ...
    constructor(
        ...
        val getAllWalletKeys: Flow<Map<String, String>> =
            context.walletKeysDataStore.data.map { preferences ->
                preferences
                    .asMap()
        ...
        val getPrimaryWalletAddress: Flow<String?> =
            context.walletKeysDataStore.data.map { preferences ->
                preferences[PRIMARY_WALLET_KEY]
            }

        suspend fun saveWalletKey(
            address: String,
            privateKey: String,
        ) {
