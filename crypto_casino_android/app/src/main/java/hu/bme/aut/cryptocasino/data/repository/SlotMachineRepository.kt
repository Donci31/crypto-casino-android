...
import hu.bme.aut.cryptocasino.data.model.slot.GameHistoryResponse
import hu.bme.aut.cryptocasino.data.model.slot.SlotConfigResponse
import hu.bme.aut.cryptocasino.data.model.slot.SpinRequest
import hu.bme.aut.cryptocasino.data.model.slot.SpinResponse
...
class SlotMachineRepository
    ...
    constructor(
        ...
        private fun <T, R> Flow<ApiResult<T>>.mapSuccess(transform: suspend (T) -> R): Flow<ApiResult<R>> =
            flow {
                collect { result ->
                    emit(
                        when (result) {
