...
async function main() {
  ...
  const casinoToken = await ethers.getContractAt(
    "CasinoToken",
    casinoTokenAddress
  );
  const casinoVault = await ethers.getContractAt(
    "CasinoVault",
    casinoVaultAddress
  );
  ...
  console.log(
    `Initial user ETH balance: ${ethers.formatEther(initialUserETHBalance)} ETH`
  );
  console.log(
    `Initial user token balance: ${ethers.formatEther(
      initialUserTokenBalance
    )} CST`
  );
  console.log(
    `Initial user vault balance: ${ethers.formatEther(
      initialUserVaultBalance
    )} CST`
  );

  const ethToPurchase = ethers.parseEther("1.0");
  console.log(
    `\nStep 1: Purchasing tokens with ${ethers.formatEther(
      ethToPurchase
    )} ETH...`
  );
  ...
  console.log(
    `Expected tokens received: ${ethers.formatEther(
      expectedTokenAmount
    )} CST (rate: ${rate})`
  );
  ...
  console.log(
    `Updated user token balance: ${ethers.formatEther(userTokenBalance)} CST`
  );

  const approveAmount = userTokenBalance;
  console.log(
    `\nStep 2: Approving vault to spend ${ethers.formatEther(
      approveAmount
    )} tokens...`
  );
  ...
  const approveTx = await casinoToken
    .connect(user)
    .approve(casinoVaultAddress, approveAmount);
  await approveTx.wait();
  console.log(`Transaction hash: ${approveTx.hash}`);

  const allowance = await casinoToken.allowance(
    user.address,
    casinoVaultAddress
  );
  ...
  console.log(
    `\nStep 3: Depositing ${ethers.formatEther(
      depositAmount
  ...
  console.log(`\nFinal Balances:`);
  console.log(
    `User token balance: ${ethers.formatEther(finalUserTokenBalance)} CST`
  );
  console.log(
    `User vault balance: ${ethers.formatEther(finalUserVaultBalance)} CST`
  );
