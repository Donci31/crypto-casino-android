...
async function main() {
  ...
  if (!casinoTokenAddress || !casinoVaultAddress) {
    console.error(
      "Please set CASINO_TOKEN_ADDRESS and CASINO_VAULT_ADDRESS environment variables"
    );
    return;
  }
  ...
  const CasinoToken = await hre.ethers.getContractAt(
    "CasinoToken",
    casinoTokenAddress
  );
  const CasinoVault = await hre.ethers.getContractAt(
    "CasinoVault",
    casinoVaultAddress
  );
