...
public class CasinoEventListenerService {
    ...
    private void initializeEventMaps() {
        // Token events
        eventSignatureToType.put(EventEncoder.encode(CasinoToken.TOKENSPURCHASED_EVENT), BlockchainTransaction.TransactionType.TOKEN_PURCHASED);
        eventSignatureToEvent.put(EventEncoder.encode(CasinoToken.TOKENSPURCHASED_EVENT), CasinoToken.TOKENSPURCHASED_EVENT);

        eventSignatureToType.put(EventEncoder.encode(CasinoToken.TOKENSEXCHANGED_EVENT), BlockchainTransaction.TransactionType.TOKEN_EXCHANGED);
        eventSignatureToEvent.put(EventEncoder.encode(CasinoToken.TOKENSEXCHANGED_EVENT), CasinoToken.TOKENSEXCHANGED_EVENT);

        // Vault events
        eventSignatureToType.put(EventEncoder.encode(CasinoVault.DEPOSIT_EVENT), BlockchainTransaction.TransactionType.DEPOSIT);
        eventSignatureToEvent.put(EventEncoder.encode(CasinoVault.DEPOSIT_EVENT), CasinoVault.DEPOSIT_EVENT);

        eventSignatureToType.put(EventEncoder.encode(CasinoVault.WITHDRAWAL_EVENT), BlockchainTransaction.TransactionType.WITHDRAWAL);
        eventSignatureToEvent.put(EventEncoder.encode(CasinoVault.WITHDRAWAL_EVENT), CasinoVault.WITHDRAWAL_EVENT);

        eventSignatureToType.put(EventEncoder.encode(CasinoVault.BETPLACED_EVENT), BlockchainTransaction.TransactionType.BET);
        eventSignatureToEvent.put(EventEncoder.encode(CasinoVault.BETPLACED_EVENT), CasinoVault.BETPLACED_EVENT);

        eventSignatureToType.put(EventEncoder.encode(CasinoVault.WINPAID_EVENT), BlockchainTransaction.TransactionType.WIN);
    ...
    private void subscribeToEvents() {
        web3j.logsNotifications(Arrays.asList(tokenAddress, vaultAddress), Collections.emptyList())
                .subscribe(
                        event -> processEvent(event.getParams().getResult()),
    ...
    private List<Type> extractIndexedParameters(Event event, Log eventLog) {
        ...
        for (int i = 0; i < indexedParameters.size() && i + 1 < topics.size(); i++) {
            ...
            Type param = FunctionReturnDecoder.decodeIndexedValue(
                    topic,
